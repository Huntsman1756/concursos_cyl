...
$validationWriteState = Join-Path $temporaryRoot 'validation-write-state'
...
$validationWriteContract = Join-Path $temporaryRoot 'validation-write-contract.json'
...
try {
    ...
    @{
        objective = 'Capture the bounded artifact generated by validation.'
        allowedPaths = @('scripts/result.txt')
        validationCommands = @('cmd /c exit 0')
        validationMayWriteAllowedPaths = $true
        frontierPlan = 'Do not edit before validation; capture its bounded generated artifact.'
        acceptanceCriteria = @('The generated artifact reaches Frontier review.')
        budgetProfile = 'small'
        modelProfile = 'mechanical'
    } | ConvertTo-Json -Depth 5 | Set-Content -LiteralPath $validationWriteContract -Encoding utf8
    $validationWritePlan = @(@{
        exitCode=0
        changedPaths=@()
        postValidationChangedPaths=@('scripts/result.txt')
        validationExitCode=0
        jsonl=$jsonl
    }) | ConvertTo-Json -Depth 8 -Compress
    $validationWriteOutput = & $supervisor -ContractPath $validationWriteContract -StateDirectory $validationWriteState `
        -WorktreeParent $worktrees -MaxAttempts 1 -TestMode `
        -MockWorkerPlans @($validationWritePlan) -MockFrontierDecisions $acceptDecision
    if ($LASTEXITCODE -ne 0) { throw "Expected generated-artifact ACCEPT exit 0, got $LASTEXITCODE" }
    $validationWriteResult = $validationWriteOutput | ConvertFrom-Json
    if ($validationWriteResult.status -ne 'COMPLETE' -or $validationWriteResult.attempts[0].changedPaths[0] -ne 'scripts/result.txt') {
        throw 'Supervisor did not propagate and accept the validation-generated artifact.'
    }
    Write-Host 'PASS: frontier supervisor propagates bounded validation-generated artifacts to ACCEPT' -ForegroundColor Green
