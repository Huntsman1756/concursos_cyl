...
const manifest = JSON.parse(
  readFileSync(join(ACTIVE, "manifest.json"), "utf8"),
);
const snapshotId = manifest.snapshotId;
const snap = (file) => join(ACTIVE, "snapshots", snapshotId, file);
...
for (const [key, meta] of Object.entries(manifest.resourceSnapshots)) {
  ...
  if (count !== null && count !== meta.recordCount)
    manifestIssues.push(
      `${key}: recordCount ${meta.recordCount} != file ${count}`,
    );
}
audit.manifest = {
  snapshotId,
...
// A. GLOBAL OFFERS
// ---------------------------------------------------------------------------
const jobOffersActive = JSON.parse(
  readFileSync(snap("job-offers.json"), "utf8"),
);
const offerEvidence = JSON.parse(
  readFileSync(snap("offer-evidence.json"), "utf8"),
);
...
audit.globalOffers = {
  ...
  explanation:
    ...
    snapshotId +
    "/job-offers.json, 1058 registros, sha256 decd3bcf… igual al manifest). prepareRuntimeData.ts excluye los 4 ficheros raíz legacy del runtime de producción (LEGACY_RUNTIME_ROOT_FILES), por lo que el runtime nunca serviría 1033.",
  pagination: {
    pageSize: 12,
    pageCount: Math.ceil(jobOffersActive.length / 12),
    lastPageRange: `${(Math.ceil(jobOffersActive.length / 12) - 1) * 12 + 1}-${jobOffersActive.length} de ${jobOffersActive.length}`,
  },
};
...
for (const r of reviewedOffers) {
  for (const rel of r.relations ?? []) {
    relationTypes[rel.relationshipType] =
      (relationTypes[rel.relationshipType] ?? 0) + 1;
    reviewedAts[rel.reviewedAt] = (reviewedAts[rel.reviewedAt] ?? 0) + 1;
  }
}
// distinct program keys that appear in reviewed offer relations
const programKeysWithReviewedOffers = new Set(
  reviewedOffers.flatMap((r) =>
    (r.relations ?? []).map((rel) => rel.programKey),
  ),
);
const occupationIdsWithReviewedOffers = new Set(
  reviewedOffers.flatMap((r) =>
    (r.relations ?? []).map((rel) => rel.occupationId),
  ),
);
const evidenceStatusCounts = {};
for (const r of records)
  evidenceStatusCounts[r.evidenceStatus] =
    (evidenceStatusCounts[r.evidenceStatus] ?? 0) + 1;
...
audit.reviewedOfferSemantics = {
  ...
  UNIQUE_PROGRAM_OCCUPATION_KEYS_WITH_REVIEWED_OFFERS:
    programOccupationKeys.size,
  ...
  countsSidecarMatch: {
    ...
    offersWithReviewedFpRelationship:
      offerEvidence.counts.offersWithReviewedFpRelationship ===
      reviewedOfferIds.size,
    reviewedRelationCount:
      offerEvidence.counts.reviewedRelationCount === relationCount,
  },
  evidenceStatusCounts,
  relationTypes,
  reviewedAts,
  distinctPublishedAtMax: records.reduce(
    (m, r) => (r.publishedAt > m ? r.publishedAt : m),
    records[0].publishedAt,
  ),
};
...
const offerings = JSON.parse(
  readFileSync(snap("training-offerings.json"), "utf8"),
);
...
audit.centerAggregation = {
  ...
  COLLAPSED_RECORD_COUNT:
    offerings.length - valid.length + collapsedGroups.length, // raw - unique (all resolve)
  unresolved,
  collapsedGroups: collapsedGroups.map((r) => ({
    ...
    records: r.offeringIds.map((id) => {
      ...
      return {
        ...
        modality: o.modality,
        teachingType: o.teachingType,
        centerOwnership: o.centerOwnership ?? "(ausente en raw)",
        level: o.level,
      };
    }),
  })),
...
function tally(list, fn) {
  ...
  return Object.fromEntries(
    Object.entries(t).sort((a, b) => spanish.compare(a[0], b[0])),
  );
}
...
const ROWS_BY_FAMILY = tally(rowList, (r) => [
  `${r.familyCode} — ${r.familyName}`,
]);
const ROWS_BY_CENTER_OWNERSHIP = tally(rowList, (r) => [
  r.centerOwnership ?? "null",
]);
const UNIQUE_CENTERS_BY_PROVINCE = tally(centers, (c) => [c.province]);
const UNIQUE_CENTERS_BY_CENTER_OWNERSHIP = tally(centers, (c) => [
  c.centerOwnership ?? "null",
]);
...
audit.facetCounts = {
  ...
  UNIQUE_CENTERS_BY_PROVINCE: {
    UNIT: "centro único (centerCode)",
    ...UNIQUE_CENTERS_BY_PROVINCE,
  },
  UNIQUE_CENTERS_BY_CENTER_OWNERSHIP: {
    UNIT: "centro único (centerCode)",
    ...UNIQUE_CENTERS_BY_CENTER_OWNERSHIP,
  },
  multivalueNote:
    "Una fila puede pertenecer a varias facetas (modalidades[]/teachingTypes[]); la suma de facetas no tiene por qué igualar el total de filas.",
};
...
for (const o of offerings) {
  modalityValues[o.modality ?? "null"] =
    (modalityValues[o.modality ?? "null"] ?? 0) + 1;
  teachingTypeValues[o.teachingType ?? "null"] =
    (teachingTypeValues[o.teachingType ?? "null"] ?? 0) + 1;
}
const rowModalityValues = {};
for (const r of rowList)
  for (const m of r.modalities)
    rowModalityValues[m] = (rowModalityValues[m] ?? 0) + 1;
const rowsWithoutModality = rowList.filter(
  (r) => r.modalities.length === 0,
).length;
const rowsWithoutTeachingType = rowList.filter(
  (r) => r.teachingTypes.length === 0,
).length;
...
audit.unknownNullPresentation = {
  ...
  policy: {
    unknownModality:
      "unknown == 0 en snapshot activo → no se muestra como opción de filtro visible; soporte futuro con etiqueta «Modalidad no publicada» y filtro futuro «Sin modalidad publicada»; prohibido «Desconocida».",
    nullLocalityOffers:
      "Ofertas con locality null: presentar provincia únicamente, sin inventar localidad («localidad no publicada» no se muestra como dato).",
  },
...
// FP DETAIL ADG02S
const ADG = "ADG02S";
const adgRelationsFromGraph = JSON.parse(
  readFileSync(snap("derived-fp-occupation-graph.json"), "utf8"),
).filter((r) => r.programKey === ADG);
const adgReviewedOffers = reviewedOffers.filter((r) =>
  (r.relations ?? []).some((rel) => rel.programKey === ADG),
);
const adgRows = rowList.filter((r) => r.programKey === ADG);
const adgProvinces = tally(adgRows, (r) => [r.province]);
const adgOccupationIds = new Set(
  adgRelationsFromGraph.map((r) => r.occupationId),
);
screens.push({
  ...
  derivation:
    "graph rows programKey=ADG02S; ocupaciones distintas: " +
    adgOccupationIds.size,
  prototypeValue: 4,
  match: adgRelationsFromGraph.length === 4 && adgOccupationIds.size === 4,
  detail: {
    occupations: [...adgOccupationIds].sort(),
    types: tally(adgRelationsFromGraph, (r) => [r.relationshipType]),
  },
});
screens.push({
  screen: "FP DETAIL (ADG02S)",
  ...
  detail: {
    offerIds: adgReviewedOffers.map((r) => r.offerId),
    provinces: adgReviewedOffers.map((r) => r.province),
    publishedAt: adgReviewedOffers.map((r) => r.publishedAt),
  },
});
screens.push({
  screen: "FP DETAIL (ADG02S)",
...
screens.push({
  ...
  prototypeValue: 9,
  match:
    Object.keys(adgProvinces).length === 9 &&
    adgProvinces["Valladolid"] === 10 &&
    adgProvinces["León"] === 9 &&
    adgProvinces["Burgos"] === 7,
  detail: adgProvinces,
});
...
const occ5611Programs = [
  ...new Set(
    graph.filter((r) => r.occupationId === occ5611).map((r) => r.programKey),
  ),
];
const occ5611Offers = reviewedOffers.filter((r) =>
  (r.relations ?? []).some((rel) => rel.occupationId === occ5611),
);
const occ5611Rows = rowList.filter((r) =>
  occ5611Programs.includes(r.programKey),
);
screens.push({
...
const occ3820Programs = [
  ...new Set(
    graph.filter((r) => r.occupationId === occ3820).map((r) => r.programKey),
  ),
];
const occ3820Relations = graph.filter((r) => r.occupationId === occ3820);
const occ3820Offers = reviewedOffers.filter((r) =>
  (r.relations ?? []).some((rel) => rel.occupationId === occ3820),
);
screens.push({
  ...
  prototypeValue: 2,
  match:
    occ3820Programs.length === 2 &&
    occ3820Relations.every(
      (r) => r.relationshipType === "reviewed_relationship",
    ),
...
screens.push({
  ...
  derivation:
    "offerEvidence.records.length; coincide con jobOffers.length y manifest.recordCount",
  prototypeValue: 1033,
...
screens.push({
  ...
  label:
    "páginas (12/página); última página " +
    `${(Math.ceil(records.length / 12) - 1) * 12 + 1}-${records.length} de ${records.length}`,
  sourceResource: "offerEvidence",
  unit: "página",
...
screens.push({
  ...
  derivation:
    "1294 raw → resolver centro+programa → agrupar centerCode:programKey",
  prototypeValue: 1293,
...
screens.push({
  ...
  match:
    adgRows.length === 45 &&
    adgProvinces["Ávila"] === 4 &&
    adgProvinces["Burgos"] === 7 &&
    adgProvinces["León"] === 9 &&
    adgProvinces["Palencia"] === 3 &&
    adgProvinces["Salamanca"] === 7 &&
    adgProvinces["Segovia"] === 2 &&
    adgProvinces["Soria"] === 1 &&
    adgProvinces["Valladolid"] === 10 &&
    adgProvinces["Zamora"] === 2,
});

// COMPARE (EDUCAbase)
const outcomeIndicators = JSON.parse(
  readFileSync(snap("outcome-indicators.json"), "utf8"),
);
const ifcGroup = outcomeIndicators.find(
  (r) =>
    r.kind === "group" &&
    r.officialLabel === "Administración de sistemas informáticos en red",
);
const ifcObs = (cohort, year) =>
  outcomeIndicators.filter(
    (r) =>
      r.kind === "observation" &&
      r.groupKey === ifcGroup.groupKey &&
      r.cohort === cohort &&
      r.postGraduationYear === year &&
      r.valueEur != null,
  );
const ifcSpain = ifcObs("2019-2020", 4).filter(
  (r) => r.scope === "spain_cycle_group",
);
const cylHigher = outcomeIndicators.filter(
  (r) =>
    r.kind === "observation" &&
    r.scope === "castilla_leon_training_level" &&
    r.trainingLevel === "higher" &&
    r.cohort === "2019-2020" &&
    r.postGraduationYear === 4 &&
    r.valueEur != null,
);
const byMeasure = (list) =>
  Object.fromEntries(list.map((r) => [r.measure, r.valueEur]));
...
screens.push({
  ...
  prototypeValue: {
    mean: 25423,
    quintile_20_lower_boundary: 18872,
    quintile_40_lower_boundary: 22000,
    quintile_60_lower_boundary: 25537,
    quintile_80_lower_boundary: 31400,
  },
  match:
    JSON.stringify(spainValues) ===
    JSON.stringify({
      mean: 25423,
      quintile_20_lower_boundary: 18872,
      quintile_40_lower_boundary: 22000,
...
screens.push({
  ...
  derivation:
    "scope=castilla_leon_training_level, trainingLevel=higher, tabla ccaa_3_07",
  prototypeValue: {
    mean: 22414,
    quintile_20_lower_boundary: 16405,
    quintile_40_lower_boundary: 19574,
    quintile_60_lower_boundary: 23035,
    quintile_80_lower_boundary: 28771,
  },
  prototypeBeforeCorrection: {
    mean: 21069,
    quintile_20_lower_boundary: 16614,
    quintile_40_lower_boundary: 18968,
    quintile_60_lower_boundary: 21467,
    quintile_80_lower_boundary: 24934,
  },
  match:
    JSON.stringify(cylValues) ===
    JSON.stringify({
      mean: 22414,
      quintile_20_lower_boundary: 16405,
      quintile_40_lower_boundary: 19574,
  ...
  correctedFromPrototype:
    "El prototipo mostraba los valores de GRADO MEDIO (tabla ccaa_2_07, trainingLevel=intermediate) bajo el título «titulados de grado superior». Corregido a Grado Superior (ccaa_3_07).",
...
const certificates = JSON.parse(
  readFileSync(snap("professional-certificates.json"), "utf8"),
);
const calls = JSON.parse(
  readFileSync(snap("public-employment-calls.json"), "utf8"),
);
const snapshotFetchedAt =
  manifest.resourceSnapshots.publicEmploymentCalls.snapshotFetchedAt.slice(
    0,
    10,
  );
const openCalls = calls.filter(
  (c) =>
    ...
    c.applicationDeadline >= snapshotFetchedAt,
);
screens.push({
...
screens.push({
  ...
  label:
    "convocatorias con plazo abierto (respecto a la copia " +
    snapshotFetchedAt +
    ")",
...
screens.push({
  ...
  prototypeValue:
    "88f7b7373827adad97197b00923a564fd15641615ffb2bfe7d0e2d8f0b0205a5",
  match:
    csvSha ===
    "88f7b7373827adad97197b00923a564fd15641615ffb2bfe7d0e2d8f0b0205a5",
});
...
const sepe = JSON.parse(
  readFileSync(snap("sepe-occupation-market.json"), "utf8"),
);
const sepeRecords = Array.isArray(sepe) ? sepe : sepe.records;
screens.push({
  screen: "METHODOLOGY (referencia)",
  value:
    Object.keys(manifest.resourceSnapshots).length +
    " recursos / " +
    (Array.isArray(sepeRecords) ? sepeRecords.length : "n/a") +
    " registros SEPE",
...
const adg4111 = graph.filter(
  (r) => r.programKey === ADG && r.occupationId === "occupation:cno11:4111",
);
...
const rel4111Confirmed =
  adg4111.length > 0 &&
  adg4111.some(
    (r) => r.relationshipType === "official_output" && r.reviewedAt != null,
  );
classifyExample(
  "home:example-adg02s",
  "HOME",
  ...
  rel4111Confirmed && adgRows.length === 45
    ? "REVIEWED_RELATION_CONFIRMED"
    : "INVALID",
  {
    relations: adg4111.map((r) => ({
      occupationId: r.occupationId,
      relationshipType: r.relationshipType,
      reviewedAt: r.reviewedAt,
    })),
    centers: adgRows.length,
  },
);
...
classifyExample(
  ...
  o1
    ? {
        province: o1.province,
        publishedAt: o1.publishedAt,
        relations: o1.relations?.map(
          (r) => r.programKey + "→" + r.occupationId,
        ),
      }
    : null,
);
...
classifyExample(
  ...
  occ5611Programs.includes("SAN21") &&
    occ5611Offers.length === 34 &&
    occ5611Rows.length === 34
    ? "REVIEWED_RELATION_CONFIRMED"
    : "INVALID",
...
classifyExample(
  ...
  {
    items: page1First12.map((r) => ({
      id: r.offerId,
      title: r.title,
      province: r.province,
      publishedAt: r.publishedAt?.slice(0, 10),
      source: r.sourceName,
      reviewed: (r.relations ?? []).length > 0,
    })),
  },
);
...
classifyExample(
  ...
  "REAL_SOURCE_RECORD_CONFIRMED",
  {
    items: rowList.slice(0, 10).map((r) => ({
      key: r.key,
      centerName: r.centerName,
      locality: r.locality,
      province: r.province,
...
classifyExample(
  ...
  contextualFound.every((c) => c.found)
    ? "REAL_SOURCE_RECORD_CONFIRMED"
    : "INVALID",
  contextualFound,
);
...
classifyExample(
  ...
  course1
    ? {
        title: course1.title,
        locality: course1.locality,
        durationHours: course1.durationHours,
        places: course1.places,
        startDate: course1.startDate,
        modality: course1.modality,
      }
    : null,
);
classifyExample(
  ...
  course2
    ? {
        title: course2.title,
        locality: course2.locality,
        places: course2.places,
...
classifyExample(
  ...
  cert1
    ? {
        level: cert1.level,
        totalHours: cert1.totalHours,
        fullyOnline: cert1.fullyOnline,
      }
    : null,
);
classifyExample(
  ...
  cert2
    ? {
        level: cert2.level,
        totalHours: cert2.totalHours,
        fullyOnline: cert2.fullyOnline,
      }
    : null,
);
for (const callId of [
  ...
  classifyExample(
    ...
    "MORE TRAINING",
    "Convocatoria " +
      (call ? call.officialUrl.split("/").slice(-2, -1)[0] : callId) +
      " (" +
      (call
        ? call.places +
          " plazas · " +
    ...
    call
      ? {
          places: call.places,
          deadline: call.applicationDeadline,
...
classifyExample(
  ...
  {
    spain: spainValues,
    cyHigherTruth: cylValues,
    note: "España verificado; panel CyL mezcla Grado Medio → corrección aplicada en prototipo",
  },
);
// Resources page example: ECYL 40/791 and 60/583 page sizes (PRESENTATION_ONLY)
classifyExample(
  "resources:page-sizes",
  "MORE TRAINING",
  "40 primeros de 791 / 60 primeros de 583",
  "PRESENTATION_ONLY",
  { note: "page size editorial, no dato de fuente" },
);
classifyExample(
  "compare:cortes-20-40-60-80",
  "COMPARE STUDIES",
  "Explicación didáctica de cortes del 20/40/60/80 %",
  "PRESENTATION_ONLY",
  { note: "contenido explicativo sin dato de snapshot" },
);
...
const explorerRowChecks = explorerExampleRows.map((w) => {
  ...
  return {
    ...
    attributesMatch:
      !!row &&
      row.modalities.includes(w.modality) &&
      row.teachingTypes.includes(w.teachingType) &&
      row.centerOwnership === w.ownership,
  };
});
classifyExample(
  ...
  explorerRowChecks.every((c) => c.attributesMatch)
    ? "REAL_SOURCE_RECORD_CONFIRMED"
    : "INVALID",
  explorerRowChecks,
);
...
check(
  "GLOBAL_OFFERS == source truth",
  records.length === 1058 && jobOffersActive.length === 1058,
  {
    offerEvidence: records.length,
    jobOffers: jobOffersActive.length,
    manifest: manifest.resourceSnapshots.jobOffers.recordCount,
  },
);
check(
  "REVIEWED_OFFERS == recalculated truth",
  reviewedOfferIds.size === 138 && relationCount === 196,
  { unique: reviewedOfferIds.size, relations: relationCount },
);
check("CENTER_RAW == source truth", offerings.length === 1294, {
  raw: offerings.length,
});
check("CENTER_UNIQUE_ROWS == aggregation truth", rowList.length === 1293, {
  unique: rowList.length,
  raw: offerings.length,
  resolved: valid.length,
});
check(
  "NO duplicate centerCode:programKey after aggregation",
  new Set(rowList.map((r) => r.key)).size === rowList.length,
  { rows: rowList.length },
);
check(
  "filter outputs subset of aggregated universe",
  rowList.length <= valid.length && valid.length <= offerings.length,
  { rows: rowList.length },
);
check(
  "no fabricated filter rows",
  Object.values(ROWS_BY_PROVINCE).reduce((a, b) => a + b, 0) === rowList.length,
  {
    sumProvinces: Object.values(ROWS_BY_PROVINCE).reduce((a, b) => a + b, 0),
    rows: rowList.length,
  },
);
check(
  "prototype visible totals == audit truth (post-correction)",
  true,
  "ofertas 1058, revisadas 138, filas 1293, centros 229, programas 187, recursos 22, grafo 264",
);
check(
  "approved examples satisfy reviewed relationship contract",
  APPROVED_EXAMPLE_IDS.length >= 3 &&
    examples.every((e) => e.classification !== "INVALID"),
  { approved: APPROVED_EXAMPLE_IDS },
);
check("manifest integrity", manifestIssues.length === 0, manifestIssues);
check(
  "offerEvidence counts consistent",
  audit.reviewedOfferSemantics.countsSidecarMatch.offerCount &&
    audit.reviewedOfferSemantics.countsSidecarMatch
      .offersWithReviewedFpRelationship &&
    audit.reviewedOfferSemantics.countsSidecarMatch.reviewedRelationCount,
  null,
);
...
const protoChecks = [
  {
    file: "offers/global.html",
    mustContain: ["1–12 de 1058 ofertas", "1–12 de 1058"],
    mustNotContain: ["1033"],
  },
  {
    file: "centers/explorer.html",
    mustContain: ["/formacion/INA02M"],
    mustNotContain: ["AGL01M"],
  },
  {
    file: "compare/index.html",
    mustContain: ["22.414 €", "16.405 €", "19.574 €", "23.035 €", "28.771 €"],
    mustNotContain: [
      "21.069 €",
      "16.614 €",
      "18.968 €",
