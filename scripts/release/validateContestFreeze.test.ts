...
describe("contest coverage freeze validator", () => {
  it("accepts the synchronized Track A freeze and recomputes its figures", () => {
    ...
    expect(freeze.sourceCommitSha).toBe(
      "9a732f67dc0b9afca16f6bb631afe7887d0e5a52",
    );
    expect(freeze.coverage.distinctQualificationCount).toBe(13);
    expect(freeze.coverage.modalityKeyCount).toBe(14);
    expect(freeze.offers.matchedOfferCount).toBe(43);
