...
const fixedOptions = {
  ...
  fetchEducationCenterDirectoryRecords: async () => [
    {
...
describe("buildSnapshots", { timeout: BUILD_SNAPSHOTS_TEST_TIMEOUT }, () => {
  ...
  it("publishes the Task 5 mapping counts and identities in the checked-in snapshot", async () => {
    ...
    expect(occupations).toHaveLength(158);
  ...
  it("retains the FP one-word publication review snapshot beyond ordinary history", async () => {
    const root = await temporaryRoot();
    await buildSnapshots({ rootDirectory: root, ...fixedOptions });
    const initialSnapshot = dirname(
      assetPath(
  ...
  it("retains the FP specific-evidence review snapshot beyond ordinary history", async () => {
    const root = await temporaryRoot();
    await buildSnapshots({ rootDirectory: root, ...fixedOptions });
    const initialSnapshot = dirname(
      assetPath(
  ...
  it.each(["coverage-freeze.json", "release-evidence.json"])(
    "retains the contest snapshot referenced by %s",
    async (artifactName) => {
      const root = await temporaryRoot();
      await buildSnapshots({ rootDirectory: root, ...fixedOptions });
      const initialSnapshot = dirname(
      ...
      await writeFile(
        ...
        JSON.stringify({ manifest: { snapshotId: basename(initialSnapshot) } }),
        "utf8",
      );
