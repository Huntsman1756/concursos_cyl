# SAN02S — Prótesis Dentales

**Review date:** 2026-08-21  
**TodoFP:** <https://www.todofp.es/que-estudiar/familias-profesionales/sanidad/protesis-dentales.html>  
**Classification:** [BOE CNO-11](https://www.boe.es/eli/es/rd/2010/11/26/1591), [INE explanatory notes](https://www.ine.es/daco/daco42/clasificaciones/cno11_notas.pdf)

## Exact outputs

TodoFP records:

- «Comercial en la industria dental o depósitos dentales.»
- «Responsable técnico de un laboratorio de prótesis dental.»
- «Responsable técnico en departamentos de investigación y desarrollo de productos en la industria dental.»
- «Técnica / técnico especialista en prótesis dental.»
- «Técnica / técnico superior en prótesis dentales.»

## Accepted relations

| Key     | CNO-11 label | Type                                                         | Evidence quote          | Rationale                                                |
| ------- | ------------ | ------------------------------------------------------------ | ----------------------- | -------------------------------------------------------- |
| `SAN02S | 3316`        | Técnicos en prótesis dentales                                | `official_output`       | «Técnica / técnico especialista en prótesis dental.»     | This is the direct official TodoFP output for the dental-prosthesis technician function, so it is the one direct-output row in this wave.                 |
| `SAN02S | 2640`        | Profesionales de ventas técnicas y médicas (excepto las TIC) | `reviewed_relationship` | «Comercial en la industria dental o depósitos dentales.» | The dental-industry sales output is technical/medical product sales. 2640 preserves that boundary; it is not promoted to a generic commercial occupation. |

No alias is created.

## Rejected alternatives

|   Code | Reason                                                                                                                                                      |
| -----: | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `3314` | Clinical diagnostic laboratory technicians do not represent fabrication or technical sales of dental prostheses.                                            |
| `3315` | Orthoprosthesis is not dental prosthesis.                                                                                                                   |
| `3124` | Electronics is not a dental-prosthesis or dental technical-sales occupation.                                                                                |
| `3510` | Agentes y representantes comerciales is generic commercial work and loses the technical/medical product boundary established by the dental-industry output. |

The two accepted rows do not infer relationships for the two responsible or
management outputs; those remain unpublished.
